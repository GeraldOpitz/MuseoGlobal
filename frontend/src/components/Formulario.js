...
const Formulario = ({ modo, setObras, obraSeleccionada, setObraSeleccionada }) => {
  ...
  useEffect(() => {
    // Si hay una obra seleccionada, actualiza los campos del formulario con sus valores
    if (obraSeleccionada) {
      ...
      // Si no hay obra seleccionada, limpia los campos del formulario
      setNombre('');
      setAutor('');
      setFechaCreacion('');
      setPaisProcedencia('');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.get('http://localhost:8080/api/obras'); // Actualiza la lista de obras
